...
const OrdersInRangeGraph = () => {
    ...
    if (startDate && endDate) {
      const fetchData = async () => {
        try {
          ...
          // ✅ Adjust to use "rows" inside the response
  ...
  // ✅ Updated field names (totalAmount, orderCount)
  const totalOrders = chartData.reduce((sum, item) => sum + (item.orderCount || 0), 0);
  const totalAmount = chartData.reduce((sum, item) => sum + (item.totalAmount || 0), 0);
  ...
  const chartOptions = {
    ...
    tooltip: {
      y: { formatter: val => `KD ${val.toFixed(3)}` }
    },
    stroke: { curve: 'smooth' },
    grid: { show: true }
  };
  ...
  const series = [
    {
      name: t("totalAmount"), // localized label
